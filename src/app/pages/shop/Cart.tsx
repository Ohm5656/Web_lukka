...
import { ArrowRight, ShieldCheck, ShoppingCart, Trash2 } from "lucide-react";
        ...
        <motion.div initial={{ opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }}>
          <div className="mb-8 flex items-center gap-3">
            ...
            <div>
              ...
              <p className="mt-1 text-sm text-slate-500">ตัวอย่าง flow ตะกร้าสำหรับเว็บขายสินค้าคอมแบบหลายหน้า</p>
            </div>
          </div>
          ...
          {cartItems.length > 0 ? (
                      ...
                      <li key={item.id} className="grid grid-cols-1 items-center gap-4 p-4 sm:p-6 md:grid-cols-12">
                        <div className="col-span-1 flex items-center gap-4 md:col-span-6">
                          ...
                          <div className="min-w-0 flex-1">
                            ...
                            <p className="mt-2 text-xs leading-relaxed text-slate-500">{item.bestFor}</p>
                          </div>
                        </div>
                        ...
                        <div className="col-span-1 flex items-center md:col-span-3 md:justify-center">
                          <div className="flex h-10 w-28 items-center overflow-hidden rounded-xl border border-slate-200 bg-slate-50">
                            <button className="flex h-full w-1/3 items-center justify-center text-slate-500 transition-colors hover:bg-slate-200 hover:text-slate-900">-</button>
                            <input type="text" value="1" readOnly className="h-full w-1/3 border-none bg-transparent p-0 text-center text-sm font-semibold text-slate-950 focus:ring-0" />
                            <button className="flex h-full w-1/3 items-center justify-center text-slate-500 transition-colors hover:bg-slate-200 hover:text-slate-900">+</button>
                          </div>
                        </div>
                        ...
                        <div className="col-span-1 flex items-center justify-between md:col-span-2 md:block md:text-right">
                          ...
                          <span className="font-bold text-slate-950">{formatPrice(item.price)}</span>
                        </div>

                        <div className="col-span-1 flex justify-end">
                          <Button variant="ghost" size="icon" className="h-9 w-9 rounded-full text-slate-400 hover:bg-red-50 hover:text-red-500">
                ...
                <div className="sticky top-24 rounded-[1.75rem] border border-slate-200 bg-white p-6 shadow-sm">
                  ...
                  <div className="mt-5 rounded-xl border border-slate-100 bg-slate-50 p-3 text-xs text-slate-500">
                    <div className="flex items-center gap-2">
                      ...
                      <span>Flow นี้เป็นเดโมหน้าเว็บ แต่โครงสร้างพร้อมต่อยอดเป็นระบบชำระเงินจริงได้</span>
                    </div>
                  </div>
                </div>
              </div>
            ...
            <div className="rounded-[1.75rem] border border-dashed border-slate-200 bg-white py-20 text-center">
              ...
              <p className="mx-auto mt-3 max-w-md text-slate-500">ลองกลับไปดูหมวดสินค้า หรือเริ่มจากบทความแนะนำก่อนแล้วค่อยเลือกสินค้าที่ตรงกับการใช้งานก็ได้</p>
