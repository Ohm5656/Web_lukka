...
export function ProductDetail() {
  ...
  const categoryInfo = categories.find((item) => item.id === product.categoryId);
  ...
  const relatedGuide =
    ...
    articles.find((article) =>
      article.relatedProducts.some((relatedProductId) => products.find((item) => item.id === relatedProductId)?.categoryId === product.categoryId)
    ) ||
    articles[0];
    ...
    <div className="min-h-screen bg-slate-50 pb-20">
      <div className="border-b border-slate-200 bg-white">
        <div className="container mx-auto flex items-center gap-2 px-4 py-4 text-sm text-slate-500 md:px-6">
          ...
          <span>/</span>
          <Link to={`/products?category=${product.categoryId}`} className="hover:text-sky-700">{categoryInfo?.name ?? product.category}</Link>
          <span>/</span>
