  ...
  return (
    <div className="min-h-screen bg-slate-50">
      <section className="relative overflow-hidden bg-slate-950 py-20 text-white">
        ...
        <div className="container relative mx-auto px-4 md:px-6">
          <motion.div initial={{ opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }} className="max-w-3xl">
            ...
            <h1 className="font-display text-4xl font-bold tracking-tight md:text-5xl">อ่านก่อนซื้อ เพื่อเลือกให้ตรงกับการใช้งานจริง</h1>
            <p className="mt-5 text-lg leading-relaxed text-slate-300">
              รวมคู่มือสำหรับมือใหม่ บทความเปรียบเทียบ และคำแนะนำที่ช่วยให้เข้าใจสเปกได้ง่าย ไม่รู้สึกว่ากำลังอ่านข้อมูลเทคนิคที่ยากเกินไป
            </p>
      ...
      <section className="container mx-auto px-4 py-12 md:px-6 md:py-16">
        <div className="mb-8 flex gap-3 overflow-x-auto border-b border-slate-200 pb-3">
          {articleTabs.map((tab) => (
            <button
              ...
              className={`rounded-full px-4 py-2 text-sm font-medium transition-colors ${activeTab === tab ? "bg-sky-100 text-sky-800" : "text-slate-500 hover:bg-slate-100 hover:text-slate-900"}`}
            >
              {tab}
            </button>
          ))}
        ...
        {featuredArticle ? (
          <>
            <motion.div initial={{ opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }} className="grid gap-8 rounded-[2rem] border border-slate-200 bg-white p-6 shadow-sm md:grid-cols-2 md:p-8">
              <div className="overflow-hidden rounded-[1.5rem]">
                <img src={featuredArticle.image} alt={featuredArticle.title} className="h-full w-full object-cover" />
              </div>
              <div className="flex flex-col justify-center">
                <Badge variant="secondary" className="w-fit bg-sky-100 text-sky-800">{featuredArticle.category}</Badge>
              ...
              {restArticles.length > 0 ? (
                <div className="grid gap-6 md:grid-cols-2 xl:grid-cols-3">
                  {restArticles.map((article, index) => (
                    <motion.div key={article.id} initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: index * 0.05 }}>
                ...
                <div className="rounded-[1.75rem] border border-dashed border-slate-300 bg-white px-6 py-14 text-center">
                  <h3 className="text-xl font-semibold text-slate-950">ยังไม่มีบทความที่ตรงกับคำค้นหา</h3>
                  <p className="mt-2 text-sm text-slate-600">ลองเปลี่ยนคำค้นหาหรือเลือกหมวดบทความอื่นดูได้</p>
                </div>
              )}
            </div>
          </>
          ...
          <div className="rounded-[1.75rem] border border-dashed border-slate-300 bg-white px-6 py-14 text-center">
            ...
            <p className="mt-2 text-sm text-slate-600">ลองล้างคำค้นหาหรือเลือกหมวดอื่นดูได้</p>
          </div>
        )}
      </section>
    </div>
