...
export function Products() {
  ...
  React.useEffect(() => {
    ...
    async function loadCatalog() {
      try {
        ...
        const [nextProducts, nextCategories, nextArticles] = await Promise.all([
          fetchProducts(),
          fetchCategories(),
          fetchArticles(),
        ]);
        ...
        setError("ยังโหลดข้อมูลสินค้าไม่สำเร็จ กรุณาลองใหม่อีกครั้ง");
      } finally {
        if (isMounted) {
          setIsLoading(false);
        }
  ...
  }, [activeCategory, products, searchQuery, showFeaturedOnly, showNewOnly, sortBy]);
    ...
    return articles
      .filter((article) =>
        article.relatedProducts.some(
          (productId) => products.find((product) => product.id === productId)?.categoryId === activeCategory,
        ),
      )
      .slice(0, 2);
  }, [activeCategory, articles, products]);
        ...
        <div className="container relative z-10 mx-auto px-4 md:px-6">
          <motion.div initial={{ opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }} className="max-w-3xl">
            ...
            <h1 className="font-display text-4xl font-bold tracking-tight text-slate-950 md:text-5xl">
              {categoryInfo ? categoryInfo.name : "เลือกสินค้าที่เหมาะกับคุณ"}
            </h1>
            <p className="mt-4 text-lg leading-relaxed text-slate-600">
              {categoryInfo
                ? categoryInfo.description
                : "รวมสินค้าทั้งหมดของร้านไว้ในหน้าเดียว พร้อมระบบค้นหาและจัดเรียงให้เลือกต่อได้ง่ายขึ้น"}
            </p>
          </motion.div>
        </div>
      </section>
      ...
      <div className="container mx-auto px-4 py-10 md:px-6 md:py-12">
        <div className="grid gap-8 lg:grid-cols-[280px_minmax(0,1fr)]">
              ...
              <div className="mt-4 space-y-2">
                <button
                  ...
                  className={`w-full rounded-2xl px-4 py-3 text-left text-sm font-medium transition-colors ${
                    !activeCategory ? "bg-sky-100 text-sky-800" : "bg-slate-50 text-slate-700 hover:bg-slate-100"
                  }`}
                >
                  สินค้าทั้งหมด
                </button>
          ...
          <div>
            ...
            <div className="mt-6">
              {isLoading ? (
                <div className="rounded-[1.75rem] border border-slate-200 bg-white px-6 py-16 text-center">
                  <h3 className="text-xl font-semibold text-slate-950">กำลังโหลดรายการสินค้า</h3>
                  <p className="mt-2 text-sm text-slate-600">ดึงข้อมูลจากระบบหลังบ้านอยู่ตอนนี้</p>
                </div>
              ) : error ? (
                <div className="rounded-[1.75rem] border border-dashed border-rose-200 bg-white px-6 py-16 text-center">
                  <h3 className="text-xl font-semibold text-slate-950">{error}</h3>
            ...
            {relatedArticles.length > 0 && !isLoading && !error && (
              <div className="mt-16 rounded-[2rem] border border-slate-200 bg-white p-6 shadow-sm md:p-8">
                <div className="mb-8 flex flex-col gap-2 md:flex-row md:items-end md:justify-between">
                  <div>
                    ...
                    <p className="mt-2 text-sm text-slate-600">รวมบทความที่เชื่อมกับหมวดสินค้านี้ไว้ให้อ่านต่อได้ทันที</p>
                  </div>
