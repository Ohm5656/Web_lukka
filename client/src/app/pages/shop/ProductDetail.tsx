...
import { fetchArticles, fetchCategories, fetchProduct, fetchProducts } from "../../lib/catalog-api";
...
export function ProductDetail() {
  ...
  React.useEffect(() => {
    ...
    async function loadProductDetail() {
      try {
        ...
        const [nextProduct, nextProducts, nextCategories, nextArticles] = await Promise.all([
          fetchProduct(id),
          fetchProducts(),
          fetchCategories(),
          fetchArticles(),
        ...
        setError("ยังโหลดรายละเอียดสินค้าไม่สำเร็จ กรุณาลองใหม่อีกครั้ง");
      } finally {
        if (isMounted) {
          setIsLoading(false);
        }
  ...
  const relatedGuide = React.useMemo(() => {
    if (!product || articles.length === 0) {
      return null;
    }

    return (
      articles.find((article) => article.relatedProducts.includes(product.id)) ||
      articles.find((article) =>
        article.relatedProducts.some(
          (relatedProductId) => products.find((item) => item.id === relatedProductId)?.categoryId === product.categoryId,
        ),
      ) ||
      articles[0]
    );
  }, [articles, product, products]);

  if (isLoading) {
    return (
      <div className="min-h-screen bg-slate-50 pb-20">
        <div className="container mx-auto px-4 pt-12 text-center md:px-6">
          <h1 className="text-3xl font-bold text-slate-950">กำลังโหลดรายละเอียดสินค้า</h1>
          <p className="mt-3 text-slate-600">ดึงข้อมูลจากระบบหลังบ้านอยู่ตอนนี้</p>
        </div>
      </div>
    );
  }
  ...
  if (error || !product) {
    return (
      <div className="min-h-screen bg-slate-50 pb-20">
        <div className="container mx-auto px-4 pt-12 text-center md:px-6">
          <h1 className="text-3xl font-bold text-slate-950">{error ?? "ไม่พบข้อมูลสินค้า"}</h1>
    ...
    <div className="min-h-screen bg-slate-50 pb-20">
      <div className="border-b border-slate-200 bg-white">
        <div className="container mx-auto flex items-center gap-2 px-4 py-4 text-sm text-slate-500 md:px-6">
          ...
          <span>/</span>
          <Link to={`/products?category=${product.categoryId}`} className="hover:text-sky-700">
        ...
        <div className="grid gap-10 lg:grid-cols-[1.02fr_0.98fr]">
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.05 }}>
            <div className="rounded-[2rem] border border-slate-200 bg-white p-7 shadow-sm">
              ...
              <h1 className="mt-4 font-display text-3xl font-bold tracking-tight text-slate-950 md:text-4xl">
                {product.name}
              </h1>
              ...
              <div className="mt-6 flex items-end gap-4">
                <div className="w-28">
                  ...
                  <div className="flex h-12 items-center overflow-hidden rounded-xl border border-slate-200 bg-slate-50">
                    <button
                      ...
                      className="flex h-full w-1/3 items-center justify-center text-slate-500 hover:bg-slate-200"
                    >
                      -
                    </button>
                    <input
                      value={quantity}
                      readOnly
                      className="h-full w-1/3 bg-transparent text-center text-sm font-semibold text-slate-900 outline-none"
                    />
                    <button
                ...
                <Button
                  size="lg"
                  className="h-12 flex-1 rounded-xl shadow-lg shadow-sky-600/10"
                  onClick={() =>
                    addToCart({
                      id: product.id,
                      name: product.name,
          ...
          <div className="mb-8 flex overflow-x-auto border-b border-slate-200">
            {[
              ...
              <button
                ...
                className={`border-b-2 px-5 py-4 text-sm font-semibold transition-colors ${
                  activeTab === tab.key ? "border-sky-600 text-sky-700" : "border-transparent text-slate-500 hover:text-slate-900"
                }`}
              >
                {tab.label}
              </button>
          ...
          <div className="rounded-[2rem] border border-slate-200 bg-white p-6 shadow-sm md:p-8">
            ...
            <ul className="mt-5 space-y-3">
              {[
                ...
                "มีรุ่นใกล้เคียงให้เปรียบเทียบทันที",
