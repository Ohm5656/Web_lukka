...
import { fetchContactInfo, type ContactInfo } from "../lib/site-api";
...
export function Footer() {
  ...
  React.useEffect(() => {
    ...
    fetchContactInfo()
      ...
      .catch(() => {
        // Keep the footer stable even if the contact API is temporarily unavailable.
      });
        ...
        <div className="grid gap-10 lg:grid-cols-[1.5fr_1fr_1fr_1fr]">
          <div>
            ...
            <p className="mt-4 max-w-md text-sm leading-relaxed text-neutral-600">
              TechHaven ร้านขายอุปกรณ์คอม พร้อมบทความและข้อมูลแนะนำ
              <br />
              สำหรับคนที่อยากเลือกให้คุ้มและตรงการใช้งาน
            </p>
            <div className="mt-5 flex gap-3 text-neutral-400">
              {[Facebook, Instagram, Twitter].map((Icon, index) => (
                <a
                  key={index}
                  href="#"
                  className="flex h-10 w-10 items-center justify-center rounded-full border border-neutral-200 bg-white transition-colors hover:border-red-200 hover:text-red-700"
                >
          ...
          <div>
            ...
            <ul className="mt-4 space-y-3 text-sm text-neutral-600">
              <li>
                <Link to="/products" className="transition-colors hover:text-red-700">
                  สินค้าทั้งหมด
                </Link>
              </li>
              {categories.slice(0, 4).map((category) => (
                <li key={category.id}>
                  <Link to={`/products?category=${category.id}`} className="transition-colors hover:text-red-700">
                    {category.name}
                  </Link>
                </li>
              ))}
          ...
          <div>
            ...
            <ul className="mt-4 space-y-3 text-sm text-neutral-600">
              <li>
                <Link to="/articles" className="transition-colors hover:text-red-700">
                  บทความความรู้
                </Link>
              </li>
              <li>
                <Link to="/faq" className="transition-colors hover:text-red-700">
                  คำถามที่พบบ่อย
                </Link>
              </li>
              <li>
                <Link to="/about" className="transition-colors hover:text-red-700">
                  เกี่ยวกับร้าน
                </Link>
              </li>
              <li>
                <Link to="/contact" className="transition-colors hover:text-red-700">
                  ติดต่อเราและข้อมูลหน้าร้าน
                </Link>
              </li>
            </ul>
          </div>
